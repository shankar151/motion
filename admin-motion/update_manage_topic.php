<?php
		...
		if(isset($_GET['upd']))
		{
			...
			$qs="select * from manage_topic inner join course on selected_course=course_id where mt_id='$x'";
			
			$data=mysqli_query($con,$qs);
			$row1=mysqli_fetch_array($data) or die(mysqli_error($con));
			//echo var_dump($row1);
		}
		...
		if(isset($_POST['update']))
		{
			...
			$qs="update manage_topic set selected_course='$course_id',unit_name='$unit_name',topic_name='$topic_name',unit='$unit',upload_pdf='$pdf',video_url='$video_url' where mt_id='$x'";
					...
					<!--mobile dropdown menu-->
						...
						<div class="col-lg-12">
							<nav class="breadcrumb_widgets" aria-label="breadcrumb mb30">
								<h4 class="title float-left">Update Topics</h4>
												    	...
												    	<input type="hidden" class="form-control" id="formGroupExampleInput1" placeholder="UX/UI" name="assig_course_id">
															<select name="course_id" class="form-control" >
																<?php
																	...
																	{
																		if($row1['selected_course']==$row['course_id'])
																		{
													...
													<div class="my_profile_setting_input form-group">
												    	...
												    	<input type="hidden" class="form-control" id="formGroupExampleInput1" placeholder="UX/UI" name="assig_course_id">
															<select name="unit_name" class="form-control" >
																<?php
																	$qs="select * from manage_unit";
																	$data=mysqli_query($con,$qs);
																	
																	while($row=mysqli_fetch_array($data,MYSQLI_BOTH))
																	{
																		if($row['unit_sr_no']==$row1['unit'])
																		{
											...
											<div class="row">
													...
													<div class="my_profile_setting_input form-group">
												    	<label for="formGroupExampleInputX">Topic Name</label>
															...
															<input type="text" class="form-control" name="topic_name" placeholder="Topic name" value="<?php echo $row1['topic_name']; ?>" required>
													</div>
												</div>
															...
															<select name="unit" class="form-control" >
																<?php
																	...
																	while($row=mysqli_fetch_array($data,MYSQLI_BOTH))
																	{
																		...
																		if($row['unit_sr_no']==$row1['unit'])
																		{
																		echo "<option value='".$row['unit_sr_no']."' selected>
																						".$row['unit_sr_no']."
																					</option>";
																		}
																		else
																		{
																			echo "<option value='".$row['unit_sr_no']."'>
																						".$row['unit_sr_no']."
																					</option>";
																		}
																	}
													...
													<div class="my_profile_setting_input form-group">
												    	<label for="formGroupExampleInputX">Upload PDF</label>
															...
															<input type="file" class="form-control" name="upload_pdf" placeholder="Unit sr no." value="<?php echo $row1['upload_pdf']; ?>" required>
													</div>
												</div>
												...
												<div class="col-xl-6">
													<div class="my_profile_setting_input form-group">
												    	<label for="formGroupExampleInputX">Video Url</label>
															...
															<input type="text" class="form-control" name="video_url" placeholder="video_url" value="<?php echo $row1['video_url']; ?>" required>
													</div>
												</div>
